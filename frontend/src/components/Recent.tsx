...
export default function RecentExpense() {
  ...
  async function fetchExpenses() {
    try {
      const res = await axios.get(
        `${BASE.replace(/\/$/, "")}/api/v1/expense/allexpense`,
        {
          headers: { "Content-Type": "application/json", ...authHeader() },
        }
      );
      const data: Expense[] = (res.data?.allExpense ?? res.data ?? []).map(
        (e: any) => ({
          ...e,
  ...
  const handleAddExpense = async () => {
    ...
    try {
      ...
      const newExpense: Expense = {
        ...
        date: res.data?.newExpense?.date
          ? new Date(res.data.newExpense.date).toLocaleDateString()
  ...
  async function saveEdit() {
    ...
    try {
      ...
      const updated: Expense = {
        ...
        date: res.data?.updatedExpense?.date
          ? new Date(res.data.updatedExpense.date).toLocaleDateString()
          : undefined,
      };

      setExpenses((prev) =>
        prev.map((ex) => ((ex._id ?? "") === editingId ? updated : ex))
      );
      setEditingId(null);
      setEditFields({});
    } catch (err) {
  ...
  async function handleDelete(id?: string) {
    ...
    try {
      await axios.delete(
        `${BASE.replace(/\/$/, "")}/api/v1/expense/delete/${id}`,
        {
          headers: { ...authHeader() },
        }
      );
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-3 justify-start w-full md:w-auto">
            ...
            <select
              ...
              className="bg-[#1e1e1e] border border-[#282828] rounded-lg px-2 py-1.5 text-white text-sm focus:outline-none focus:ring-1 focus:ring-[#26e07f] transition h-8"
            >
              {categories.map((cat) => (
            ...
            <button
              ...
              className="flex items-center gap-2 bg-[#1e1e1e] border border-[#282828] rounded-lg px-2 py-1 sm:px-3 sm:py-1.5 text-xs sm:text-sm font-medium text-white hover:bg-[#282828] transition"
            >
              <span className="flex items-center justify-center w-5 h-5 rounded-full bg-[#26e07f] text-black font-bold text-xs sm:text-sm">
                +
              </span>
              Add Expense
            </button>
            ...
            <div className="relative group">
              <button className="flex items-center gap-2 bg-[#1e1e1e] border border-[#282828] rounded-lg px-2 py-1 sm:px-3 sm:py-1.5 text-xs sm:text-sm font-medium text-white hover:bg-[#282828] transition">
                <img
                  width="16"
                  height="12"
                  src="https://img.icons8.com/windows/32/FFFFFF/share-rounded.png"
                  alt="share-rounded"
                  className=" w-5 h-4"
                />
                Share
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-700">
                <th className="text-left text-gray-400 font-semibold pb-4 pr-1">
                  Description
                </th>
                <th className="text-left text-gray-400 font-semibold pb-4 pr-1">
                  Category
                </th>
                <th className="text-left text-gray-400 font-semibold pb-4">
                  Date
                </th>
                <th className="text-right text-gray-400 font-semibold pb-4">
                  Amount
                </th>
                <th className="text-right text-gray-400 font-semibold pb-4">
                  Actions
            ...
            <tbody>
              {filteredExpenses.map((exp) => (
                <tr
                  key={exp._id ?? `${exp.title}-${exp.amount}`}
                ...
                >
                  ...
                  <td className="py-4 text-right">
                    <button
                      ...
                      className="text-gray-400 hover:text-white mr-3 transition px-3 py-1 rounded-lg hover:bg-gray-700"
                    >
                      Edit
                    </button>
                    <button
                      onClick={() => handleDelete(exp._id)}
                      className="text-gray-400 hover:text-red-400 transition px-3 py-1 rounded-lg hover:bg-gray-700"
                    >
                      Delete
                    </button>
                  </td>
            ...
            <div className="bg-[#171717] border border-[#282828] rounded-2xl p-6 w-[90%] max-w-md">
              ...
              <div className="flex flex-col gap-4">
                ...
                <label className="flex items-center gap-2 text-gray-300">
                  <input
                    type="checkbox"
                    checked={Boolean(editFields.recurring)}
                    onChange={(e) =>
                      setEditFields({
                        ...editFields,
